...
## 5.1 Automation (roles, playbooks, CLI, config)
...
## 5.3 Public Content (`public-content/`)
...
Cluster inventory lives in `secrets/cluster-credentials.yml` (Ansible
Vault encrypted), not in the `config/` directory.
...
1. Environment variables for CI pipelines
2. Local YAML files for developer workstations (file provider with `file://` references)
3. Kubernetes Secrets for in-cluster execution
4. AWS Secrets Manager
5. Optional HashiCorp Vault support
...
ACM integration is optional. When disabled, ACM registration and
ACM-related lab activities are skipped. Enable by setting `enabled: true`
and providing a `hub_cluster_id` that references a cluster in the inventory.
