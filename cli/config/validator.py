...
class ConfigValidator:
                ...
                errors.append(
                    {
                        "severity": "ERROR",
                        "message": (
                            f"Expected {config.fleet.facilitator_cluster_count}"
                            f" facilitator clusters, "
    ...
    def _check_credential_profiles(self, config: MASWorldConfig) -> list[dict[str, Any]]:
        ...
        if config.student_credentials.default_profile not in profile_names and profile_names:
            errors.append(
                {
                    "severity": "ERROR",
                    "message": (
    ...
    def _scan_dict_for_secrets(self, data: Any, path: str, errors: list[dict[str, Any]]) -> None:
    ...
    def _check_shared_password_in_event(self, config: MASWorldConfig) -> list[dict[str, Any]]:
        ...
        if config.student_credentials.allow_shared_password:
            errors.append(
                {
                    "severity": "WARNING",
                    "message": ("Shared student passwords enabled — not suitable for event use"),
    ...
    def _check_cluster_connections(
        ...
        for cluster in clusters:
            if not cluster.enabled:
                continue
            if not cluster.connection.api_url:
                errors.append(
                    {
                        "severity": "WARNING",
                        "message": f"Cluster '{cluster.id}' has no API URL (not yet provisioned?)",
