...
class ClusterConnection(BaseModel):
    api_url: str = ""
    admin_auth_method: AuthMethod = AuthMethod.PASSWORD
    admin_username: str = "cluster-admin"
...
class ClusterConfig(BaseModel):
    ...
    connection: ClusterConnection = ClusterConnection()
...
class ACMComponentConfig(ComponentConfig):
    enabled: bool = False
    hub_cluster_id: str = ""
...
class ComponentsConfig(BaseModel):
    ...
    acm_registration: ACMComponentConfig = ACMComponentConfig()
...
class MASWorldConfig(BaseModel):
    ...
    student_credential_profiles: dict[str, StudentCredentialProfile] = Field(default_factory=dict)
    ...
    def validate_attendee_restrictions(self) -> MASWorldConfig:
        for name, profile in self.student_credential_profiles.items():
            ...
            if profile.restrictions.allow_cluster_admin:
                raise ValueError(f"Profile '{name}' grants cluster-admin to attendees — forbidden")
        return self
